       ...
       alert('some error occurred.');
        ...
        $.ajax({
          ...
          success:function(data){
            ...
            {
              var url = "<?=base_url();?>/enrollment/print_receipt/" + data;
              window.open(url, "_blank");
            // Create a button element and set its href attribute
              var button = $('<a>')
                  .attr('href', '<?= base_url(); ?>/enrollment/print_receipt/' + data)
                  .attr('target', '_blank')
                  .addClass('btn btn-success btn-xs')
                  .text('Print Receipt');

              // Append the button to the print-btn div
              $('#print-btn').empty().append(button);
              $('.tr_form').css('display','none');
              var pay_button = $('#h_s_pay_button'+id).html();
             if (pay_button=='Cancel') { $('#h_s_pay_button'+id).html('Pay'); }
              //location.reload();
            }
...
$(document).ready(function(){
    ...
    $('#msg').html('');
      ...
      $('#tableDATA').load('<?=base_url()?>enrollment/reception_fees_data/'+stu_id+'/'+Scholarship+'/'+FEE_PLAN);
